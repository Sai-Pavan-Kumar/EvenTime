    ...
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 bg-slate-50/50">
        ...
        <div className="flex gap-1">
          <button type="button" onClick={prevMonth} className="w-8 h-8 rounded-lg flex items-center justify-center text-[#6C47FF] hover:bg-[#6C47FF]/10 transition-colors">‹</button>
          <button type="button" onClick={nextMonth} className="w-8 h-8 rounded-lg flex items-center justify-center text-[#6C47FF] hover:bg-[#6C47FF]/10 transition-colors">›</button>
        </div>
      </div>
      <div className="px-4 py-3">
        ...
        <div className="grid grid-cols-7 gap-[2px]">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button key={d} type="button" disabled={past} onClick={() => onSelect(new Date(viewYear, viewMonth, d))} className={["aspect-square flex items-center justify-center text-[13px] rounded-lg transition-all", past ? "text-slate-300 cursor-not-allowed" : "cursor-pointer", selected ? "bg-[#6C47FF] text-white font-bold shadow-lg shadow-[#6C47FF]/30 scale-110 rounded-xl" : todayCell ? "font-bold text-[#6C47FF]" : !past ? "text-slate-700 hover:bg-[#6C47FF]/10 hover:text-[#6C47FF] hover:font-semibold" : ""].join(" ")}>{d}</button>
            );
          })}
        </div>
      </div>
            ...
            {results.map((r, i) => {
              ...
              return (
                <button key={i} type="button" onClick={() => handleSelect(r)} className="w-full flex items-start gap-3 px-4 py-3 hover:bg-[#6C47FF]/5 transition-colors text-left border-b border-slate-100 last:border-0">
                  <MapPin className="w-4 h-4 text-[#6C47FF] mt-0.5 shrink-0" />
