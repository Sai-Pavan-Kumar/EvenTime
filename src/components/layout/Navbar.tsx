...
import { Search, Plus, User, LogOut, Trophy, Settings, SquarePlus, Building2, Home, X, Bug, CalendarDays, BarChart2, ChevronDown } from "lucide-react";
...
interface NavbarProps {
  ...
  categoryChips?: ChipType[];
  locationChips?: ChipType[];
...
function NavbarInner({ 
  ...
  const effectiveCategoryChips: ChipType[] = categoryChips.length > 0
    ? categoryChips
    : categoriesList.map((c) => ({ name: c, value: c }));
  const effectiveLocationChips: ChipType[] = locationChips.length > 0
    ? locationChips
    : CITIES.map((c) => ({ name: c, value: c }));
  const hasFilterChips = effectiveCategoryChips.length > 0 || effectiveLocationChips.length > 0;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 gap-4 relative">
          ...
          <div className={`${variant === 'centered' ? 'absolute left-1/2 -translate-x-1/2 flex items-center gap-2 shrink-0' : 'flex items-center gap-2 shrink-0'}`}>
            <Link href="/" className="shrink-0 flex items-center gap-2">
              <div className="w-9 h-9 relative shrink-0"> 
                <Image src="/logo1.webp" alt="logo" fill sizes="100px" className="object-contain" priority />
              </div>
          ...
          {variant !== 'centered' && (
            <>
              <form ref={desktopSearchRef} onSubmit={handleSearch} className="flex-1 max-w-lg relative hidden sm:block mx-5">
                ...
                <input
                  ...
                  maxLength={100}
                  className="w-full bg-white border border-[rgba(0,0,0,0.08)] shadow-sm rounded-full pl-10 pr-9 py-2.5 text-sm font-['Switzer',sans-serif] text-text-primary focus:border-brand-primary focus:ring-4 focus:ring-brand-primary/15 outline-none placeholder:text-text-secondary transition-all"
                />
                {searchQuery.length > 0 && (
                  <button
                    type="button"
                    onClick={() => setSearchQuery("")}
                    className="absolute right-3.5 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 transition-colors flex items-center justify-center cursor-pointer"
            ...
            <Link 
              href="/cities" 
              className={`flex items-center gap-2 text-sm font-['Outfit'] transition-colors shrink-0 ${
                pathname === '/cities' || pathname.startsWith('/cities/') 
                  ? 'text-brand-primary font-black' 
                  : 'text-text-secondary hover:text-brand-primary font-bold'
              }`}
            >
              ...
              <Link 
                ...
                className={`flex items-center gap-2 text-sm font-['Outfit'] transition-colors shrink-0 ${
                  pathname === '/leaderboard' 
                    ...
                    : 'text-text-secondary hover:text-amber-500 font-bold'
                }`}
              >
